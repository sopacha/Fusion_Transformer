...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
            ...
            args = [c2, args[1]]
        elif m is EnhancedGPT:
            c2 = args[0]
            if isinstance(f, list):
                if len(f) == 2:  # For two-stream input
                    args = [ch[f[0]]]    
        elif m is EnhancedGPTWithCrossAttention:
            c2 = args[0]
            if isinstance(f, list):
                if len(f) == 2:  # For two-stream input (RGB and IR)
                    # Ensure both input streams have the same channel count
                    c1_rgb = ch[f[0]]  # Channel count for RGB stream
                    c1_ir = ch[f[1]]   # Channel count for IR stream
            
                    # If channels are different, add channel adjustment
                    if c1_rgb != c1_ir:
                        # Option 1: Force matching to the larger channel count
                        c2 = max(c1_rgb, c1_ir)
                
                        # Potentially add a preprocessing conv to match channels
                        # This might involve adding a conv layer before the EnhancedGPTWithCrossAttention
                        # You could do this by modifying the model construction logic
                    else:
                        c2 = c1_rgb
            
            # Update args to reflect the channel count
            args = [c2]
        elif m is Detect:
